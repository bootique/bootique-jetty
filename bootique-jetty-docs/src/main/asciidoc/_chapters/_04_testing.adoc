...
----
<1> `JettyTester` doesn't have a need for lifecycle annotations, so you just need to create one instance per BQRuntime.
<2> `JettyTester` helps to bootstrap a test BQRuntime. It resets all connectors configured in the app,
and created a connector on a randomly selected unoccupied port.

NOTE: Not hardcoding the connector port improves your chances that the test would work regardless of
what else is running on the same machine. Often it also reduces the amount of test configuration (previously
...
----
<1> JettyHelper provides access to an HTTP client "target", so we don't need to worry about the dynamic port or service
host name.
<2> JettyHelper has a number of static methods that help with response assertions.