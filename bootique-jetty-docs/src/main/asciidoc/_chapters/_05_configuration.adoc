...
|`cors`
|none
|Configures CORS support. CORS is only activated when at least one `cors.urlPatterns` entry is set. See `jetty.cors` below.
|===
...
"jetty.cors" configures CORS support. CORS is only installed when at least one `urlPatterns` entry is present, so applications that don't use it incur no overhead. To apply CORS to the entire context, set `urlPatterns` to `/*`. Example:
...
|`urlPatterns`
|empty (CORS disabled)
|URL patterns CORS applies to. At least one pattern must be set for CORS to be activated; use `/*` to apply CORS to the entire context. CORS is only applied to matching requests, and bypassed for all others.
|===
