...
----
By default each "static servlet" shares the common `staticResourceBase`, but it can also define its own base using a
...
`resourceBase` simply overrides `staticResourceBase`, while `pathInfoOnly` controls static resource URL resolution:

|===
|`pathInfoOnly`|URL|File Path
